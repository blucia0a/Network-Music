...
if [ "${NETWORK_MUSIC_WIFI_KEY_FILE}" = "" ]; then
  NETWORK_MUSIC_WIFI_KEY_FILE="${HOME}/.network-music/keyfile"
  echo "Monitoring traffic on $1 with authentication in NETWORK_MUSIC_WIFI_KEY_FILE at $NETWORK_MUSIC_WIFI_KEY_FILE"
fi

if [ ! -f ${NETWORK_MUSIC_WIFI_KEY_FILE} ]; then
  echo "$NETWORK_MUSIC_WIFI_KEY_FILE does not exist!"
  exit 1
fi
